local function aesthetic_record()
    ...
    local full_path = vim.fn.expand("~/Movies/Terminal/") .. output_filename
...
local function aesthetic_screenshot()
    ...
    local full_path = vim.fn.expand("~/Pictures/Terminal/") .. output_filename

    local cmd = string.format(
        [[ffmpeg -y -f avfoundation -i "3" -frames:v 1 -filter:v "crop=1920:1080:(in_w-1920)/2:(in_h-1080)/2,unsharp=5:5:1.0:5:5:0.0" "%s"]],
        full_path
    )
